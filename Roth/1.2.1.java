...
//Added: tryPushPirate with spesific pirate and location
//Changed: BasicV2 Tactic
//MUST CHECK IF THE ANTICAMPER WORKS FINE FOR BOTH WAYS
            ...
            for (Pirate p : game.getMyLivingPirates()) {
                if (p.hasCapsule()) {
                    if (pirate.canPush(p) && p.distance(myMothership) <= 900) {
                        this.tryPushPirate(p, game.getMyMothership()); // pushes capsuler to the motherbase
                    }
                }
            }
        }
