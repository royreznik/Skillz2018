...
//nearestWall(MapObject obj) might need to be static, currently its not.
...
class Engine
{
     ...
     */
    public Location nearestWall(MapObject obj) {
...
class BotPirate {
    ...
    public void antiCamperLogic() {
        ...
        for (Pirate p : game.getMyLivingPirates()) {
            if (p.hasCapsule()) { // if the pirate has a capsule
                if (campersOnRadius) { //if there are campers
                    if (p.distance(myMothership) <= 900) // only if the push will result in a point
                        this.tryPushPirate(p, myMothership); // pushes capsuler to the motherbase
                    return;
                }
