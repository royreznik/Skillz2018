...
public class BotPirate {
    ...
    public void tryPushPirate(Pirate p, MapObject obj) {
        if (p != null && !this.didPush && this.pirate.canPush(p) && p.isAlive()) {
            // Push the enemy!
    ...
    public void tryPushAsteroid(Asteroid a, MapObject obj) {
        if (!this.didPush && pirate.canPush(a) && a.turnsToRevive == 0 && game.pushDistance > 0 && a.isAlive() && a != null) {
            // Push the enemy!
    ...
    public void tryPushWormhole(Wormhole w, MapObject obj) {
        if (!this.didPush && pirate.canPush(w) && w != null) {
            // Push the Wormhole!
...
//
//    public void bombOnMe() {
//        game.debug("Bomb on - " + this.pirate.stickyBombs[0].carrier + " | bomb#" + this.pirate.stickyBombs[0]);
//        if (!this.pirate.hasCapsule() || Engine.minTurnsToDest(this.pirate, Engine.getClosestMyMothership(this.pirate)) > Engine.boomIn(this.pirate)) {
//            if (Engine.getClosestLivingAsteroid(this.pirate) != null)
//                tryPushAsteroid(Engine.getClosestLivingAsteroid(this.pirate), Engine.pushAsteroidTo(Engine.getClosestAsteroid(this.pirate)));
//
//            if (Engine.turnsToDest(this.pirate, Engine.getClosestEnemyCapsuler(this.pirate)) < Engine.boomIn(this.pirate)) {
//                destination = Engine.getClosestEnemyCapsuler(this.pirate);
//
//            } else if (Engine.turnsToDest(this.pirate, Engine.getClosestEnemy(this.pirate)) < Engine.boomIn(this.pirate)
//                    && this.pirate.stickyBombs.length > 0
//                    && Engine.friendsInRange(Engine.getClosestEnemy(this.pirate), Engine.firstBombToBlowUp(this.pirate).explosionRange) <= 1)
//                // maybe we should use turnsToDest() and not minTurnsToDest, need to implement minRoute for an exFriend in navigator.
//                destination = Engine.getClosestEnemy(this.pirate);
//            else if (Engine.turnsToDest(this.pirate, Engine.nearestWall(this.pirate)) < Engine.boomIn(this.pirate))
//                destination = Engine.nearestWall(this.pirate);
//            else if (Engine.turnsToDest(this.pirate, Engine.getClosestLivingAsteroid(this.pirate)) < Engine.boomIn(this.pirate))
//                destination = Engine.getClosestLivingAsteroid(this.pirate);
//            else destination = Engine.getClosestMyMothership(this.pirate);
//        } else destination = Engine.getClosestMyMothership(this.pirate);
//    }
...
////                destination = Engine.getClosestEnemyCapsuler(this.pirate);
////            } else if (Engine.turnsToDest(this.pirate, Engine.getClosestEnemy(this.pirate)) < this.pirate.stickyBombs[0].countdown
////                    && Engine.friendsInRange(Engine.getClosestEnemy(this.pirate), Engine.firstBombToBlowUp(this.pirate).explosionRange) <= 1)
////                // maybe we should use turnsToDest() and not minTurnsToDest, need to implement minRoute for an exFriend in navigator.
////                destination = Engine.getClosestEnemy(this.pirate);
////            else
...
//        }
        ...
        if (numOfEnemyShips < 2 && numOfEnemyShips > 0) {
            enemyCapsuler = Engine.getClosestEnemyCapsuler(this.pirate);
...
//                List<Pirate> enemyBombers = Engine.enemyPiratesWithBomb();
                ...
                if (target != null && target.hasCapsule()
                        && (Engine.willPushToWallKill(this.pirate, target)
                        || (Engine.getClosestFriend(this.pirate) != null && Engine.getClosestFriend(this.pirate).canPush(target)))) {
                    tryPushPirate(target, Engine.nearestWall(target));
                }
//                if (enemyBombers.size() > 0) {
//                    Collections.sort(enemyBombers, (p1, p2) -> p1.distance(this.pirate) - p2.distance(this.pirate));
//                    if (enemyBombers.get(0).distance(this.pirate) < game.stickyBombExplosionRange) {
//                        if (enemyBombers.get(0).stickyBombs[0].countdown < 2) {
//                            tryPushPirate(enemyBombers.get(0), Engine.nearestWall(enemyBombers.get(0)));
//                        }
//                    }
//                }
                if (target == null) {
                    if (Engine.bombNeeded(enemyCapsuler)) {
                        if (enemyCapsuler.distance(game.getEnemyMotherships()[0]) < 1200) {
                            if (!didPush && !PirateHandler.getBombed()) {
                                //tryStickyBomb(enemyCapsuler);  
...
//        Pirate shahid = Engine.getClosestShahid(this.pirate);
//        if (shahid != null && shahid.distance(this.pirate) < Engine.biggestBombRange(shahid)) {
//            StickyBomb s = Engine.firstBombToBlowUp(shahid);
//            if (Engine.shouldPushShahid(this.pirate, shahid, Engine.nearestWall(shahid)))
//                tryPushPirate(shahid, Engine.nearestWall(shahid));
//            else if (Engine.getClosestLivingAsteroid(shahid) != null
//                    && shahid.distance(Engine.getClosestLivingAsteroid(shahid))
//                    - Engine.getClosestLivingAsteroid(shahid).size < this.pirate.pushDistance
//                    && Engine.enemiesInRange(shahid, s.explosionRange) < 2)
//                tryPushPirate(shahid, Engine.getClosestLivingAsteroid(shahid));
//            if (didPush) return;
//        }
        List<Pirate> enemyBombers = Engine.enemyPiratesWithBomb();
        if (enemyBombers.size() > 0) {
        ...
        /*for(Pirate capPirate : game.getMyLivingPirates())
        {
            ...
            {
                if(this.pirate.pushReloadTurns < 1)
                {
                    int pushDist = this.pirate.stateName.equals("normal") ? 600 : 1000;
                    if(game.getMyMotherships().length > 0)
                    {
                        if(game.getMyMotherships()[0].distance(capPirate) < pushDist)
                        {
                            this.tryPushPirate(capPirate,game.getMyMotherships()[0]);
                            if(didPush)
                            {
                                return;
                            }
...
//        destination = Engine.getBombingLocation(this.pirate.location, this.pirate.maxSpeed * this.pirate.stickyBombs[0].countdown);
        Mothership enemyMom = Engine.getClosestEnemyMothership(this.pirate);
    ...
    public void antiCamperLogicV2(){
        ...
        if(!assginedCapsuler.pirate.hasCapsule() || !this.pirate.canPush(assginedCapsuler.pirate) || assginedCapsuler.pirate.numPushesForCapsuleLoss < 2){
            ...
            if(!assginedCapsuler.pirate.hasCapsule()){
                ...
                if(didPush) return;
                if(this.pirate.distance(camper) < this.pirate.distance(assginedCapsuler.pirate)) destination = camper;
            }
            return;
        }
