...
public class Engine {
    ...
    public static Capsule getClosestEnemyCapsuleByIndex(Pirate p, int x) {
        List<Capsule> capsules = Arrays.asList(game.getEnemyCapsules());
        Collections.sort(capsules, (m1, m2) -> m1.distance(p) - m2.distance(p));
        return capsules.get(x);
    }
    ...
    public static Pirate getClosestEnemyCapsuler(Pirate p) {
        ...
        try {
            ...
            game.debug("ArrayIndexOutOfBoundsException in - getClosestEnemyCapsuler()");
        }
     ...
     * capsulers, returns the closest friend to a myCapsule!
     */
    public static Pirate getClosestMyCapsuler(Pirate p) {
        ...
        try {
            // for some reason game.getMyLivingPirates(0) crashes the bot -
            // ArrayIndexOutOfBoundsException
            return myPirates.get(0);
        } catch (ArrayIndexOutOfBoundsException e) {
            game.debug("ArrayIndexOutOfBoundsException in - getClosestMyCapsuler()");
        }
