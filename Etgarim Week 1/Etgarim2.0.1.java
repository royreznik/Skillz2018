 ...
 */
//-----------------------------------------------------------
// capsuler will push away astroids in his way to the nearest wall. #MarzukMasterRace
    ...
    public void capsulerLogic() {
        ...
        for (int i = 0; i < game.getAllAsteroids().length; i++) {
            if (this.pirate.canPush(game.getAllAsteroids()[i])) {
                this.pirate.push(game.getAllAsteroids()[i], Engine.nearestWall(game.getAllAsteroids()[i]));
                return;
            }
        }
        ...
        if (camper.distance(myMothership) < 2000
            ...
            destination = myMothership.getLocation().towards(Engine.getClosestMyCapsule(this.pirate).initialLocation, camperDist + 350);
            return;
        }
        if (!campersOnRadius) {
...
//Tactic
enum Tactic {
    BASIC, BASICV2, BASICV3, BASICV4, STANDARD, REPLACER;
}
