...
public class BotPirate {
    ...
    public void camperLogicV2() {
        ...
        if (game.getEnemyMotherships().length < 2) {
            if (CamperCounter < 2) {
            ...
            if(enemyCapsuler != null)
            {
                tryPushPirate(Engine.getEnemyCapsuler(), nearestWall());
            }
            else{
            ...
            if (CamperCounter < 2) {
                if (enemyCapsuler == null) {
                    ...
                    destination = game.getEnemyMotherships()[0].getLocation().towards(enemyCapsuler, 300);
                }
            } else {
                if (enemyCapsuler == null) {
    ...
    public void astroLogic() {
        ...
        Asteroid asteroid = Engine.getClosestAsteroid(this.pirate);
        ...
        for (int i = 0; i < enemys.length; i++) {
            if (asteroid != null && Engine.getClosestEnemy(enemys[i]).distance(enemys[i]) <= asteroid.size)
                ...
                if (Engine.getClosestEnemy(enemys[i]).distance(enemys[i]) <= 1000)
                    targets.add(enemys[i]);
        }
        if (targets.size() > 1)
