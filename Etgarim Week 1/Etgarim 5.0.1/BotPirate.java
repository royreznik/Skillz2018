...
public class BotPirate {
    ...
    public void sailToDest() {
        ...
        if (this.destination.getLocation().row < 0 || this.destination.getLocation().row > game.rows || this.destination.getLocation().col < 0 || this.destination.getLocation().col > game.cols) {
            game.debug("This dest Invilide");
            return;
        }
    ...
    public void camperLogicV2() {
        ...
        if (numOfEnemyShips < 2 && numOfEnemyShips > 0) {
            ...
            if (enemyCapsuler != null) {
                if (enemyCapsuler.distance(this.pirate) < 600 && this.pirate.pushReloadTurns < 1) {
        ...
        } else 
        {
            if (CamperCounter < 2) {
                if (enemyCapsuler == null) {
                    destination = game.getEnemyMotherships()[1];
                } else {
                    destination = game.getEnemyMotherships()[1].getLocation().towards(enemyCapsuler, 300);
                }
            } else  if(CamperCounter < 5){
                if (enemyCapsuler == null) {
                    destination = game.getEnemyMotherships()[1].getLocation().towards(game.getEnemyCapsules()[0], 1000);
                } else {
                    destination = game.getEnemyMotherships()[1].getLocation().towards(enemyCapsuler, 1200);
                }

            }
            else{
                if (enemyCapsuler == null) {
                    destination = game.getEnemyMotherships()[0].getLocation().towards(game.getEnemyCapsules()[0], 1000);
                } else {
                    destination = game.getEnemyMotherships()[0].getLocation().towards(enemyCapsuler, 1200);
                }
            }
        ...
        /*{
            ...
            Mothership enemyBestShip = Engine.getClosestEnemyMothershipToCapsule();
            if(CamperCounter < 3)
            {
                if (enemyCapsuler == null) {
                    destination = enemyBestShip;
                } else {
            ...
            {
                List<Mothership> enemyShips = Arrays.asList(game.getEnemyMotherships());
                for(int x = 0; x < enemyShips.size(); x++)
                {
                    if(CamperCounter < 4)
                    {
                        if (enemyCapsuler == null) {
                            ...
                            destination =enemyShips.get(x).getLocation().towards(enemyCapsuler, 301);
                        }
                    }
                }
            }
    ...
    public void astroLogic() {
        ...
        if (asteroid != null)
            ...
            this.destination = game.getAllAsteroids()[0].initialLocation.towards(this.pirate.getLocation(), game.getAllAsteroids()[0].size + 1);
        if (destination != null) {
