...
//        }else{
    ...
    public static Asteroid[] willAsteroidHit(MapObject obj){
        return Arrays.asList(game.getAllAsteroids()).stream().filter(
                a -> (a.turnsToRevive == 1 && a.initialLocation.inRange(obj, a.size)) || a.location.inRange(obj, a.size) || a.location.add(a.direction).inRange(obj, a.size)).toArray(Asteroid[]::new);
