...
//Changed: Change the starting postion, need to fix the capsuler
...
class Engine
{
    ...
    {
        List<Capsule> capsules = Arrays.asList(game.getMyCapsules()[0]);
    ...
    {
        List<Mothership> motherships = Arrays.asList(game.getMyMotherships()[0]);
...
class BotPirate {
    ...
    public Pirate getMyCapsuler() {
        ...
        for(Pirate p: game.getMyLivingPirates()){
            if(p.distance(game.getMyCapsules()[0])<closest.distance(game.getMyCapsules()[0]))
                closest=p;
        }
        return closest;
    }
    ...
    public void camperLogic() {
        ...
        if (destination != null) {
            ...
            Location motherWall = Engine.nearestWall(game.getEnemyMotherships()[0]);
        ...
        if (destination == null) {
            this.destination = game.getEnemyMotherships()[0];
        }
        Pirate enemycapsuler = Engine.getEnemyCapsuler();
        if(enemycapsuler != null && pirate.distance(enemycapsuler) > 599)
        {
            if(enemycapsuler.distance(game.getEnemyMotherships()[0]) < pirate.distance(game.getEnemyMotherships()[0]))
    ...
    public void capsulerLogic() {
        Mothership myMothership = game.getMyMotherships()[0];
        boolean campersOnRadius = false;
        int row = pirate.getLocation().row;
        int loc = pirate.getLocation().col;
        if(game.turn > 900 && game.getMyself().score < 3)
        {
            if(this.pirate.hasCapsule())
            {
                if(this.pirate.getLocation().row != game.getMyMotherships()[0].getLocation().row)
            {
                destination = new Location(game.getMyMotherships()[0].getLocation().row, loc);
            }
            else{
        ...
        if (!this.pirate.hasCapsule()) {
            ...
            destination = myMothership.getLocation().towards(game.getMyCapsules()[0].initialLocation, 899);
            return;
        } else {
//        else if(this.pirate.canPush(myHelper())){
    ...
    public void antiCamperLogic() {
        ...
        if (campersOnRadius && this.pirate.distance(myMothership) >= 801) {
            // if there are campers, go to position
            destination =myMothership.getLocation().towards(game.getMyCapsules()[0].initialLocation, 801);
