...
public class Engine {
    ...
    public static boolean bombNeeded(Pirate p) {
        ...
        double distanceFromMotherShip = enemyMotherShipDistance(p);
        if (distanceFromMotherShip == -1) {
            return false;
        }
//        if (distanceFromMotherShip / p.maxSpeed > 5) {
//            return true;
//        }
        double d = turnsToDest(p, Engine.getClosestMyMothership(p));
        if (d != 9999 && d > 5)
            return true;
        return false;
    }

    public static double enemyMotherShipDistance(Pirate p) {
        if (p == null || !p.isAlive() || game.getEnemyMotherships().length < 1) {
            return -1;
        }
//        double dist = game.getEnemyMotherships()[0].distance(p);
//        double temp = -1;
//        for(Mothership sh : game.getEnemyMotherships())
//        {
//            temp = sh.distance(p);
//            if(temp < dist)
//            {
...
//        }
//        return dist;
        return p.distance(Engine.getClosestEnemyMothership(p));
    }
...
//    }
    ...
    public static Pirate getClosestShahid(MapObject obj) {
        ArrayList<Pirate> virgins = new ArrayList<>();
        for (Pirate enemy : game.getEnemyLivingPirates())
            if (enemy.stickyBombs.length > 0)
                virgins.add(enemy);
        virgins.sort((v1, v2) -> (v1.distance(obj) - v2.distance(obj)));
        try {
            if (virgins.size() > 0) {
                for (Pirate p : virgins) {
                    if (p != null && !p.equals(obj))
                        return p;
                }
            } else
                return Engine.getClosestEnemyCapsuler(obj);
        } catch (Exception e) {
            return Engine.getClosestEnemyCapsuler(obj);
        }
        return null;
    ...
    public static Pirate friendNoMore(MapObject obj) {
        ArrayList<Pirate> exFriends = new ArrayList<>();
        Pirate temp;
        for (Pirate ex : game.getMyLivingPirates()) {
            temp = Engine.getClosestFriend(ex);
            if (ex.stickyBombs.length > 0 && temp != null && temp.distance(ex) < ex.stickyBombs[0].explosionRange)
                exFriends.add(ex);
        }
        exFriends.sort((x1, x2) -> (x1.stickyBombs[0].countdown - x2.stickyBombs[0].countdown));
        try {
            if (exFriends.size() > 0) {
                for (Pirate p : exFriends) {
                    if (p != null && !p.equals(obj))
                        return p;
                }
            }
...
//    }
    ...
    public static double turnsToDest(Pirate pirate, MapObject dest) { // this gets a Pirate and not a MapObject to prevent confusion.
        double speed = pirate.maxSpeed;
        if (dest == null) return 9999;
        if (dest instanceof Wormhole)
            return (pirate.distance(dest) - game.wormholeRange) / speed;

        if (dest instanceof Mothership)
            return (pirate.distance(dest) - game.mothershipUnloadRange) / speed;

        return pirate.distance(dest) / speed;
    }

    public static double turnsFromObjToDestForState(MapObject obj, MapObject dest, String state) {
        double speed;
        if (state.equals("heavy"))
            speed = game.heavyMaxSpeed;
        else
            speed = game.pirateMaxSpeed;
        if (dest instanceof Wormhole)
            return (obj.distance(dest) - game.wormholeRange) / speed;

        if (dest instanceof Mothership)
            return (obj.distance(dest) - game.mothershipUnloadRange) / speed;
        return obj.distance(dest) / speed;
    }

    public static int biggestBombRange(Pirate p) {
        ArrayList<StickyBomb> bombs = new ArrayList<>(Arrays.asList(p.stickyBombs));
        if (p.stickyBombs.length > 0) {
            bombs.sort((b1, b2) -> (b2.explosionRange - b1.explosionRange));
            try {
                return bombs.get(0).explosionRange;
            } catch (Exception e) {
                return 0;
            }
        } else return 0;
    ...
    public static StickyBomb biggestBombSticked(Pirate p) {
        ArrayList<StickyBomb> bombs = new ArrayList<>(Arrays.asList(p.stickyBombs));
        if (p.stickyBombs.length > 0) {
            bombs.sort((b1, b2) -> (b2.explosionRange - b1.explosionRange));
            try {
                ...
                return p.stickyBombs[0];
            }
        } else return null;
    }

    public static int boomIn(Pirate p) {
        ArrayList<StickyBomb> bombs = new ArrayList<>(Arrays.asList(p.stickyBombs));
        if (p.stickyBombs.length > 0) {
            bombs.sort((b1, b2) -> (b1.countdown - b2.countdown));
            try {
                return bombs.get(0).countdown;
            } catch (Exception e) {
                return p.stickyBombs[0].countdown;
            }
        } else return 1000;
    }

    public static List<Pirate> enemyPiratesWithBomb() {
        return Arrays.asList(Arrays.asList(game.getEnemyLivingPirates()).stream().filter(p -> p.stickyBombs.length > 0).toArray(Pirate[]::new));
    }

    public static StickyBomb firstBombToBlowUp(Pirate p) {
        ArrayList<StickyBomb> bombs = new ArrayList<>(Arrays.asList(p.stickyBombs));
        if (p.stickyBombs.length > 0) {
            bombs.sort((b1, b2) -> (b1.countdown - b2.countdown));
            try {
    ...
    public static int friendsInRange(MapObject p, int range) {
        ArrayList<Pirate> friends = new ArrayList<>(Arrays.asList(game.getMyLivingPirates()));
        int counter = 0;
        for (Pirate x : friends) {
            if (x.distance(p) <= range)
                counter++;
        }
        return counter;
    }

    public static int enemiesInRange(MapObject p, int range) {
        ArrayList<Pirate> friends = new ArrayList<>(Arrays.asList(game.getEnemyLivingPirates()));
        int counter = 0;
        for (Pirate x : friends) {
            if (x.distance(p) <= range)
                counter++;
    ...
    public static boolean shouldPushShahid(Pirate pusher, Pirate shahid, MapObject pushingLoc) {
        if (shahid != null) {
            StickyBomb s = Engine.firstBombToBlowUp(shahid);
            return Engine.enemiesInRange(
                    shahid.getLocation().towards(pushingLoc, pusher.pushDistance)
                    , s.explosionRange) >= Engine.enemiesInRange(shahid, s.explosionRange);
        }
        return false;
    }

    public static boolean willPushToWallKill(Pirate pusher, MapObject obj) {
        return obj.distance(Engine.nearestWall(obj)) <= pusher.pushDistance;
    }

//    public static double turnsToDest(MapObject obj, MapObject dest) {
//        if (dest instanceof Wormhole)
...
//                turns += Engine.turnsToDest(capsuler, wormhole);
//            } else {
//                turns += Math.max(Engine.turnsToDest(capsuler, wormhole), wormhole.turnsToReactivate);
//            }
//            turns += Engine.turnsToDest(wormhole.partner, Engine.getClosestMyMothership(wormhole));
...
//    }
    ...
    public static MapObject fastestWayToDest(Pirate pirate, MapObject destination) {
        ...
        double sailTurns = turnsToDest(pirate, destination);
        HashMap<Wormhole, Double> options = new HashMap<>();
        double turns;
        for (Wormhole wormhole : game.getAllWormholes()) {
            turns = 0;
            if (wormhole.isActive) {
                turns += Engine.turnsToDest(pirate, wormhole);
            } else {
                turns += Math.max(Engine.turnsToDest(pirate, wormhole), wormhole.turnsToReactivate);
            }
            turns += Engine.turnsFromObjToDestForState(wormhole.partner, destination, pirate.stateName);
            options.put(wormhole, turns);
        }
        Double min = Collections.min(options.values());
//        Double min2 = min + 1;
        //game.debug("Turns = " + min);
//        if (pirate.stateName.equals("heavy") && !pirate.hasCapsule()) {
//            HashMap<Wormhole, Double> options2 = new HashMap<>();
//            for (Wormhole wormhole : game.getAllWormholes()) {
//                turns = 0;
//                if (wormhole.isActive) {
//                    turns += Engine.turnsToDest(pirate, wormhole);
//                } else {
//                    if (Engine.turnsToDest(pirate, wormhole) > wormhole.turnsToReactivate
//                            && Engine.turnsFromObjToDestForState(pirate, wormhole, "normal") + 1 < Engine.turnsToDest(pirate, wormhole)) {
//                        try {
//                            pirate.swapStates(Engine.getClosestFriend(pirate));
//                            turns += Engine.turnsFromObjToDestForState(pirate, wormhole, "normal") + 1;
//                        } catch (Exception e) {
//
//                        }
//                    } else
...
//                }
//                turns += Engine.turnsFromObjToDestForState(wormhole.partner, destination, pirate.stateName);
//                options2.put(wormhole, turns);
//            }
//            min2 = Collections.min(options2.values());
//        }
        if (Math.ceil(min) < Math.ceil(sailTurns)) {
//            if (Math.ceil(min) <= Math.ceil(min2)) {
            try {
                for (Map.Entry<Wormhole, Double> entry : options.entrySet()) {
                    if (entry.getValue().equals(min)) {
...
//            }
    ...
    public static boolean helpfulWormhole(Pirate pirate, MapObject destination) {
        return minTurnsToDest(pirate, destination) != turnsToDest(pirate, destination);
    }

    public static boolean isWormholeHelpful(Pirate pirate, MapObject destination, Wormhole wormhole) {
        int turns = 0;
        if (wormhole.isActive) {
            turns += Engine.turnsToDest(pirate, wormhole);
        } else {
        ...
        return turns < turnsToDest(pirate, destination);
    }

    public static double minTurnsToDest(Pirate pirate, MapObject destination) {
        double sailTurns = turnsToDest(pirate, destination);
        if (game.getAllWormholes().length == 0)
            return sailTurns;
        HashMap<Wormhole, Double> options = new HashMap<>();
    ...
    public static MapObject pushAsteroidTo(Asteroid asteroid) {
        ...
        List<Mothership> motherships = Arrays.asList(game.getMyMotherships());
        motherships.sort((m1, m2) -> m1.distance(asteroid) - m2.distance(asteroid));
...
//    }
     ...
     * @param pushed
     * @param wormhole
     * @return true if push will reach wormhole's range and wormhole is active || if pushed cannot escape the wormhole
     * range before the wormhole reactivates
     */
    public static boolean willPushTeleport(MapObject pusher, MapObject pushed, Wormhole wormhole) {
        int dist;
        if (pusher instanceof Pirate && wormhole != null) {
            dist = pushed.distance(wormhole) - wormhole.wormholeRange;
            if (pushed instanceof Pirate) {
                return dist <= ((Pirate) pusher).pushDistance
                        && (wormhole.isActive || (Engine.turnToGetOut(pushed.getLocation().towards(wormhole,
                        ((Pirate) pusher).pushDistance), wormhole, wormhole.wormholeRange,
                        ((Pirate) pushed).maxSpeed) >= wormhole.turnsToReactivate));
            }
        }
        return false;
    }

    public static int turnToGetOut(MapObject p, MapObject obj, int range, int maxSpeed) {
        int counter = 0;
        while (p.distance(obj) <= range) {
            counter++;
            range -= maxSpeed;
        }
        return counter;
    ...
    /**
     * returns true if there's a risk of death or drop for capsuler in loc
     * @param loc location to check
     * @param numPushesForDrop number of pushes needed for capsule drop
     * @return if theres a risk
     */
    public static boolean capsulerRisk(MapObject loc, int numPushesForDrop){
        int numPushers = 0, maxPushRadius = 0;
        if(Engine.getClosestMyMothership(loc).inRange(loc, game.mothershipUnloadRange)) return false;
        for(Pirate enemy : game.getEnemyLivingPirates()){
            if(enemy.pushReloadTurns < 2 && enemy.inRange(loc, enemy.pushRange)){
                numPushers++;
                maxPushRadius+= enemy.pushDistance;
            }
        }
        return numPushers >= numPushesForDrop || loc.inRange(nearestWall(loc), maxPushRadius);
        
    }
    
    /**
     * returns true if there is a possibility for getting pushed out of the map
     * @param loc
     * @return
     */
    public static boolean deathRisk(MapObject loc){
        int maxPushRadius = 0;
        for(Pirate enemy : game.getEnemyLivingPirates()){
            if(enemy.pushReloadTurns < 2 && enemy.inRange(loc, enemy.pushRange)){
                maxPushRadius+= enemy.pushDistance;
        ...
        return loc.inRange(nearestWall(loc), maxPushRadius);
    }
    
    /**
     * returns the nearest corner to obj
     * @param obj
     * @return
     */
    public static Location nearestCorner(MapObject obj){
        List<Location> locations = new ArrayList<>();

        locations.add(new Location(0, 0));
        locations.add(new Location(0, game.cols));
        locations.add(new Location(game.rows, 0));
        locations.add(new Location(game.rows, game.cols));
    ...
    /**
     * returns true if the bomb is a threat for MapObject mp
     * @param mp
     * @param bomb
     * @return
     */
    public static boolean canBombHit(MapObject mp, StickyBomb bomb){
        if(bomb.carrier instanceof Pirate){
            Pirate p = (Pirate)bomb.carrier;
            int range = bomb.explosionRange + p.maxSpeed * bomb.countdown;
            return mp.inRange(bomb, range);
        }
        else if(bomb.carrier instanceof Asteroid){
            Asteroid a = (Asteroid)bomb.carrier;
            Location explosionLocation = a.location.add(a.direction.multiply(bomb.countdown));
            return mp.inRange(explosionLocation, bomb.explosionRange);
        }
        else if(bomb.carrier instanceof Wormhole){
            return mp.inRange(bomb, bomb.explosionRange);
        }
        return false;
    }
    
    /**
     * sorts the possible bombing locations 
     * @param bombingLocations
     * @return
     */
    public static List<MapObject> sortBombingLocations(Set<MapObject> bombingLocations){
        Map<MapObject, Integer> friendlyDeath = new HashMap<>(), enemyDeath = new HashMap<>();
        Set<MapObject> destroyAsteroid = new HashSet<>();
        for(MapObject loc : bombingLocations){
            int friendCounter = 0, enemyCounter = 0;
            if(loc.inMap()){
                for(Pirate friend : game.getMyLivingPirates()){
                    if(friend.inRange(loc, game.stickyBombExplosionRange)) friendCounter++;
                }
                for(Pirate enemy : game.getEnemyLivingPirates()){
                    if(enemy.inRange(loc, game.stickyBombExplosionRange)) enemyCounter++;
                }
                for(Asteroid a : game.getLivingAsteroids()){
                    if(!isAsteroidMoving(a) && a.distance(getClosestMyMothership(a)) <= a.size){
                        if(a.inRange(loc, game.stickyBombExplosionRange)) destroyAsteroid.add(loc);
                    }
                }
            }
            friendlyDeath.put(loc, friendCounter);
            enemyDeath.put(loc, enemyCounter);
        }
        Comparator<MapObject> sorter = Comparator.comparing(friendlyDeath::get);
        sorter = sorter.thenComparing(enemyDeath::get);
        sorter = sorter.thenComparing(l -> destroyAsteroid.contains(l) ? 0 : 1);
        return bombingLocations.stream().sorted(sorter).collect(Collectors.toList());
    }
    
    /**
     * returns best bombing location in the movement radius starting from start
     * @param start
     * @param movementRadius
     * @return
     */
    public static MapObject getBombingLocation(Location start, int movementRadius){
        Set<MapObject> bombingLocations = new HashSet<>(), possibleBombingLocations = new HashSet<>();
        possibleBombingLocations.addAll(Arrays.asList(game.getEnemyLivingPirates()));
        possibleBombingLocations.addAll(Arrays.asList(game.getLivingAsteroids()));
        possibleBombingLocations.addAll(PirateNavigator.getCircleLocations(start, movementRadius));
        for(MapObject loc : possibleBombingLocations){
            if(loc instanceof Asteroid && isAsteroidMoving((Asteroid)loc)) continue;
            MapObject edge = start.towards(loc, game.stickyBombExplosionRange);
            if(loc instanceof Location) edge = loc;
            if(edge.inRange(start, movementRadius)) {
                bombingLocations.add(edge);
            }
        }
        possibleBombingLocations.add(nearestWall(start));
        return sortBombingLocations(possibleBombingLocations).get(0);
    }
    
    public static int intTurnsTo(Pirate p, MapObject loc){
        double d = turnsToDest(p, loc) + 0.99999;
        return (int)d;
    }
    
    /**
     * returns the best mothership for capsuler to go to
     * @param capsuler
     * @return
     */
    public static Mothership getBestCapsulerMothership(Pirate capsuler){
        if(game.getMyMotherships().length == 0) return null;
        if(game.getMyMotherships().length == 1) return game.getMyMotherships()[0];
        else if(Engine.getClosestMyMothership(capsuler).inRange(capsuler, capsuler.maxSpeed * 5)) {
            return Engine.getClosestMyMothership(capsuler);
        }
        HashMap<Mothership, Integer> campers = new HashMap<>();
        for(Mothership m : game.getMyMotherships()){
            int distance = 0;
            for(Pirate p : game.getEnemyLivingPirates()){
                distance+= p.distance(m);
            }
            campers.put(m, distance);
        }
        Comparator<Mothership> sorter = Comparator.comparing(campers::get);
        sorter = sorter.thenComparing(capsuler::distance);
        return Arrays.asList(game.getMyMotherships()).stream().min(sorter).get();
    }
}