...
public class PirateNavigator {
    ...
    public static void navigate(BotPirate bp){
        ...
        else if(!bp.didPush && canPush.length == 1 && bp.pirate.canPush(canPush[0]) && canPush[0].isAlive() && canPush[0]!=null){
    ...
    private static MapObject getCurrentPath(BotPirate bp){
        ...
        if(canMove.test(currentPath)){
            if(bp.job.equals(Job.CAPSULER)) {
                canMove = getCapsulerThreat(bp);
            }
            ...
            for(StickyBomb bomb : bombs){
                if(Engine.canBombHit(currentPath, bomb)){
                    threat = bomb;
                    break;
                }
            }
            Location deviation = bp.pirate.location.towards(threat.location, -bp.pirate.maxSpeed);
            if(!deviation.inMap()){
